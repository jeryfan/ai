...
export interface Provider<TApi extends Api = Api> {
	...
	/**
	 * Required: every provider has `apiKey` auth. Even ambient-credential
	 * providers and keyless local servers provide `apiKey` auth whose `resolve()`
	 * reports whether the provider is configured. `Models.getAuth()` returns
	 * undefined when the provider is unconfigured.
	 */
...
export interface Models {
	 ...
	 * status/availability UIs catch them and render "needs re-login" instead of
	 * treating them as unconfigured.
	 */
