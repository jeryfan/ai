...
// `openai`, `@google/genai`) and returns a struct each provider composes into
// its display string. The `messageCarriesBody` flag captures the Anthropic /
// `@google/genai` happy path where the SDK already folded the body into the
// message, so providers can preserve it without double-printing.
 ...
 * `this.error`). Empty objects are treated as no body so an empty parsed body
 * does not surface as `"{}"`. The chosen body is truncated to the cap.
 */
