...
function applyThinkingLevelMetadata(model: Model<any>): void {
	...
	if (model.provider === "openrouter" && model.id.startsWith("inception/mercury-2")) {
		...
		// OpenRouter-compatible low/medium/high pass through verbatim; OpenRouter normalizes to Mercury's vocabulary.
		mergeThinkingLevelMap(model, { off: null });
	}
	...
	try {
		...
		if (data["google-vertex"]?.models) {
			for (const [modelId, model] of Object.entries(data["google-vertex"].models)) {
				...
				// do not match the official Gemini API standard pricing table. These only account
