...
const oauthToken = await getTestApiKey("anthropic");
...
describe.skipIf(!oauthToken)("Anthropic OAuth tool name normalization", () => {
	...
	it("should handle lowercase built-in tools (read, write, edit, bash)", async () => {
		// These tools use lowercase names, while Claude Code uses PascalCase.
		const readTool: Tool = {
			name: "read",
	...
	it("should NOT map find to Glob - find is not a CC tool name", async () => {
		// A lowercase "find" tool and CC's "Glob" are DIFFERENT tools.
