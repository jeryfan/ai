...
describe("builtin providers", () => {
	...
	it("resolves vertex via explicit API key plus project and location", async () => {
		const configured = createModels({
			authContext: fakeAuthContext({
		...
		const result = await configured.getAuth(model);
		expect(result?.auth.apiKey).toBe("vertex-key");
		expect(result?.source).toBe("GOOGLE_CLOUD_API_KEY");
	});
});
