 ...
 * - Tool call ID format incompatibilities
...
/**
 * Get API key for provider - checks test env first, then env vars
 */
async function getApiKey(provider: string): Promise<string | undefined> {
	const oauthKey = await getTestApiKey(provider);
...
function dumpFailurePayload(params: { label: string; error: string; payload?: unknown; messages: Message[] }): void {
	const filename = `/tmp/ai-handoff-${params.label}-${Date.now()}.json`;
	const body = {
		label: params.label,
		error: params.error,
